    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow sm:rounded-lg py-6 px-6 border border-gray-200">
                ...
                <form id="video_upload_form" method="POST" action="{{ route('videoevents.update', $event->id) }}">
                    @csrf
                    ...
                    <div class="mt-4">
                        ...
                        <select id="live_stream_id" name="live_stream_id" class="block mt-1 w-full" required>
                            <option value="" disabled>Select a Live Stream</option>
                            @foreach ($liveStreams as $stream)
                                <option value="{{ $stream->id }}" {{ $event->live_video_id == $stream->id ? 'selected' : '' }}>
                                    {{ $stream->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-4">
                        ...
                        <div id="message_quill_editor">{!! $event->message !!}</div>
                    ...
                    <div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-2 mt-4">
                        <div class="mt-4">
                            ...
                            <x-input id="available_date" class="block mt-1 w-full" type="text" name="available_date" value="{{ $event->start_available_date }} - {{ $event->end_available_date }}" />
                            <input type="hidden" name="start_available_date" id="start_available_date" value="{{ $event->start_available_date }}" />
                            <input type="hidden" name="end_available_date" id="end_available_date" value="{{ $event->end_available_date }}" />
                        </div>

                        <div class="mt-4">
                            <x-label for="start_event_time" :value="__('Start Event Time')" />
                            <x-input id="start_event_time" class="block mt-1 w-full" type="time" name="start_event_time" value="{{ $event->start_event_time }}" required />
                        </div>

                        <div class="mt-4">
                            <x-label for="end_event_time" :value="__('End Event Time')" />
                            <x-input id="end_event_time" class="block mt-1 w-full" type="time" name="end_event_time" value="{{ $event->end_event_time }}" required />
                        </div>
                    </div>
                ...
                <script>
                    ...
                    // Initialize date range picker with existing date range
                    $('#available_date').daterangepicker({
                        opens: 'left',
                        locale: {
                            format: 'YYYY-MM-DD'
                        },
                        startDate: '{{ $event->start_available_date }}',
                        endDate: '{{ $event->end_available_date }}',
                    }, function(start, end) {
